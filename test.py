...
reveal_jaxtype(x)  # Should show Float32[Tensor, "1 224 224"]

y = torch.ones(1, 224, 224, dtype=torch.int)
reveal_jaxtype(y)  # Should show Int32[Tensor, "1 224 224"]

x = x + x  # tensor binary operator
x = -x  # tensor unary operator

x32 = torch.zeros(1, dtype=torch.float32)
x64 = torch.zeros(1, dtype=torch.float64)
x64 += x32  # implicit type promotion
